...
export default function LiveClassPage({ params }: { params: { classId: string } }) {
    ...
    const getCameraPermission = async () => {
      try {
        ...
        toast({
          ...
          description: 'Please enable camera & mic permissions.',
        });
      }
    };
  ...
  const handleEndClass = () => {
      toast({ title: "Class Ended", description: "The live session has been concluded."});
      router.back();
  }
  ...
  const renderMainContent = () => {
    ...
    if (isPresentationMode) {
        ...
        return (
            <div className="relative flex-grow">
                ...
                {/* Slide Controls - Only for Teacher */}
                {userRole === 'teacher' && (
    ...
    return (
        <div className="relative flex-grow">
             ...
             {/* Drawing Toolbar - Only for Teacher */}
             {userRole === 'teacher' && (
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] bg-muted/30 rounded-lg">
      <header className="flex items-center justify-between p-3 border-b bg-card rounded-t-lg">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="h-8"><Users size={16} className="mr-1.5"/> 25</Button>
          <Button variant="destructive" size="sm" className="h-8" onClick={handleEndClass}>
              {userRole === 'teacher' ? <VideoOff size={16} className="mr-1.5"/> : <LogOut size={16} className="mr-1.5"/> }
              {userRole === 'teacher' ? "End Class" : "Leave Class"}
          </Button>
        </div>
      </header>
          ...
          <Card className="flex-grow flex flex-col border-0 rounded-none shadow-none">
            ...
            <CardFooter className="p-2 border-t">
              <div className="flex w-full gap-2">
                 ...
                 {userRole === 'student' && <Button variant="outline" size="icon" className="h-9 w-9 shrink-0"><Hand size={16}/></Button>}
              </div>
            </CardFooter>
          </Card>
